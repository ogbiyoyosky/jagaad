    ...
    try {
      ...
      const citiesWeatherResult: any = await Promise.all(
        citiesResult.map(async (city) => {
          const weatherapiInfo: any = await customGetRequest(`http://api.weatherapi.com/v1/forecast.json?key=${Env.get('WEATHER_APIKEY')}=${city.latitude},${city.longitude}&days=2`,{})
          console.log(
